...
class VideoContextCleanTest(unittest.TestCase):
    ...
    def test_cancels_timers_without_a_context(self) -> None:
        ...
        self.assertGreaterEqual(source.cancel_video_encoder_flush.call_count, 2)
        self.assertGreaterEqual(source.cancel_video_encoder_timer.call_count, 2)
    ...
    def test_cleans_synchronously_on_encode_thread(self) -> None:
        csc = Mock()
        encoder = Mock()
        source = self.make_source(csc, encoder)

        source.video_context_clean(True)

        source.cancel_video_encoder_flush.assert_called()
        source.cancel_video_encoder_timer.assert_called_once_with()
    ...
    def test_preclosed_encoder_flush_clears_pending_state(self) -> None:
        source = self.make_source()
        class ClosedEncoder:
            flush = Mock()
        ...
        encoder = ClosedEncoder()
        source._video_encoder = encoder
        source.b_frame_flush_data = encoder, None, 1, 0, 0, None
        source.cancel_video_encoder_flush = Mock(
            side_effect=lambda: setattr(source, "b_frame_flush_data", ())
        )
        source.video_context_clean = Mock(
            side_effect=lambda encode_thread: source.cancel_video_encoder_flush()
        )

        source.do_flush_video_encoder()

        source.cancel_video_encoder_flush.assert_called_once_with()
        source.video_context_clean.assert_called_once_with(True)
